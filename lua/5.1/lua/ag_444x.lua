...
ag_e444x.__index = ag_e444x

function ag_e444x:new (o)
	--local o = o or {}
	local inst={}
	...
	setmetatable(inst, ag_e444x)
	return inst
end

function ag_e444x:bus_init()
...
function ag_e444x:start_freq(setting)
	--print(":FREQ:STAR "  .. format_value(setting ,'HZ'))

end

function ag_e444x:center(cent,span)
end

function ag_e444x:bandwidth()
...
function ag_e444x:trace(setting) -- for lack of a better argument name
    local ret = {}	
	...
	self.device:write(":SENS:FREQ:STOP "  .. format_value(setting.stop ,'HZ'))
	self.device:write(":SENS:FREQ:STAR "  .. format_value(setting.start ,'HZ'))
...
function ag_e444x:range(setting) -- for lack of a better argument name
    local ret = {}	
...
function ag_e444x:marker(o) -- for lack of a better argument name
    local ret = {}
	
end

function ag_e444x:stop(setting)
	self.device:write("FB "  .. format_value(setting ,'HZ'))
end

function ag_e444x:start(setting)
	self.device:write("FA "  .. format_value(setting ,'HZ'))
end

function ag_e444x:raw(setting)
	self.device:write(setting)
end

function ag_e444x:close(setting)
	self.device:close()
end
